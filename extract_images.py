...
def get_trimmed_shot_resolution(video_path, start_ms, end_ms):
    ...
    # print('shot_res:',shot_resolutions)
    # shot_resolutions = np.asarray(shot_resolutions)
    # print('shot_res:', shot_resolutions)
    # max_shot_resolution = np.sort(shot_resolutions[::-1])
    # if max_shot_resolution == None:
    #     max_shot_resolution = [()]
    max_shot_resolution = sorted(shot_resolutions, reverse=True)    # FIXME list is sorted on height, then width, descending
    # print('max_shot_res:', max_shot_resolution)
    ...
    while done < len(list_features_path):  # repeat until all movies in the list have been processed correctly
        ...
        for j in range(2):
            ...
            if j == 0:
                ...
                # with open('/tmp/res.pickle', 'wb') as handle:
                #     pickle.dump([resolution_template, max_res_pro_shot], handle)
            else:
                print('-------------------------------------------------------')
                print('starting image extraction')
                # with open('/tmp/res.pickle', 'rb') as handle:
                #     resolution_template, max_res_pro_shot = pickle.load(handle)
                for v_path, f_path in tqdm(zip(list_videos_path, list_features_path), total=len(list_videos_path)):
                    video_name = os.path.split(v_path)[1][:-4]
                    frames_dir = os.path.join(f_path, 'frames')
                    if not os.path.isdir(frames_dir) and not os.path.isfile(os.path.join(frames_dir, '.done')):
                        ...
                        for start_frame, end_frame in tqdm(shot_timestamps):
                            ...
                            # print('max_res:',max_res_pro_shot[video_name][start_frame])
                            # print('res_temp:',resolution_template[video_name])
                            # print(max_res_pro_shot[video_name][start_frame][1] < resolution_template[video_name][1])
