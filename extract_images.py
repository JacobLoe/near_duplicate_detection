...
def get_trimmed_shot_resolution(video_path, frame_path, start_ms, end_ms, file_extension):
    vid = cv2.VideoCapture(video_path)
    shot_resolutions = []
    file_extension = file_extension.lower()
    if file_extension[0] != '.':
    ...
    if trim_frames == 'yes':
        ...
        if not os.path.isdir(frames_dir) and not os.path.isfile(os.path.join(frames_dir, '.done')):
            ...
            for start_frame, end_frame in tqdm(shot_timestamps):
                ...
                aux_res_dict[start_frame] = get_trimmed_shot_resolution(v_path, frames_path, start_frame, end_frame, file_extension)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trim_frames", default='yes', choices=('yes', 'no'), help="")
    parser.add_argument("--frame_width", type=int, help="")
