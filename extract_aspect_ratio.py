...
def save_aspect_ratio_to_csv(video_path, features_dir, file_extension, videoid):
    ...
    ar_csv_path = os.path.join(features_dir, str(videoid)+'.csv')
...
def main(features_root, file_extension, videoids, force_run):
    ...
    for videoid in tqdm(videoids):
        ...
        if not os.path.isfile(done_file_path) or not open(done_file_path, 'r').read() == done_version or force_run == 'True':
            logger.info('aspect ratio extraction results missing or version did not match, extracting aspect ratios')
